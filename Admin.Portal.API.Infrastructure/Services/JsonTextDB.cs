    ...
    public class JsonTextDB : IDataAccess
    {
        ...
        public async Task<(bool, ProfileModel)> GetLoginUserDetails(string username)
        {
            ...
            if (readText.Length > 2)
            {
                ...
                {
                    JsonDBStructure db = obj.ToObject<JsonDBStructure>();
                    ProfileModel pro = new();
                    pro = JsonConvert.DeserializeObject<ProfileModel>(JsonConvert.SerializeObject(db.Admin.Where(u => u.Username.ToString().Equals(username, StringComparison.InvariantCultureIgnoreCase)).FirstOrDefault()));
                    return (true, pro);
                }
            }
            ...
            if (readText.Length > 2)
            {
                ...
                if (obj.Count > 0)
                {
                    ...
                    if(!String.IsNullOrEmpty(tenantID) && tenantID.Length > 0)
                        lstUsers = JsonConvert.DeserializeObject<List<UserModel>>(JsonConvert.SerializeObject(db.Users.Where(u => u.Tenenats.Any(t => t.Equals(tenantID)))));
                    else 
                        lstUsers = db.Users;
                }
        ...
        public async Task<(bool, UserModel)> CreateUser(UserModel context)
        {
            ...
            if (readText.Length > 2)
            {
                ...
                {
                    JsonDBStructure db = obj.ToObject<JsonDBStructure>();
                    context.ID = "U" + Guid.NewGuid().ToString().Replace("-", "");
