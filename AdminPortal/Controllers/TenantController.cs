...
namespace Admin.Portal.API.Controllers
{
    ...
    public class TenantController : ControllerBase
    {
        ...
        public async Task<IActionResult> Delete(TenantDeleteRequest context)
        {
            if (!new AuthenticateFilter(config, dbContext).Authenticate(AccessLevel.TenantnClaims, Claims.CLAIM_DELETE_TENANT, Request.Headers[Config.HEADER_LOGIN_USER].ToString(), context.ID))
                return new Context(Messages.ACCOUNT_INSUFFICIENT_PRIVILEGES).ToContextResult((int)HttpStatusCode.Forbidden);

            IDataAccess db = ServiceInit.GetDataInstance(config, dbContext);
            bool result = await db.DeleteTenant(context);
            if (result)
